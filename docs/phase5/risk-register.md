...
| P5-R-005 | installer 漏 Host/migrations/licenses、接受开发路径或把 dirty source 误绑定为 HEAD | 20 高 | 固定 extraResources、clean-source binding、clean VM、解包白名单和 packaged negative tests | CLEAN UNSIGNED DIR/INSTALLER CONTROLLED, SIGNED RC/VM OPEN / Quality/Release |
...
- Phase 4 已由项目负责人确认历史验收通过并在 GitHub 合并，验收 SHA 为 `4ea65dc`。旧 SHA 的隔离
  ...
  Phase 5 严格超集开发回归。
- 当前 `6dae872…` clean-HEAD lint、全仓 typecheck、workspace coverage、workspace 400 discovered / 399 passed /
  1 skipped / 0 failed，以及 Desktop 34 files / 311 discovered / 310 passed / 1 skipped 通过；Native 2/2、
  Electron E2E 6/6 通过；官方 npm audit 为 Critical=0、High=0，50 样本 metrics instrumentation smoke、
...
- 快速 OCR availability 探测触发 Native Windows test SegFault，已撤回并恢复原始探测；退出侧不稳定试验也已
  撤回。[`6dae872…` clean deterministic verify](../../artifacts/phase5/6dae872/clean-verify-local-20260719-final1/verify-summary.json)
  ...
  `acceptance=false`。[remote run 29684078146](https://github.com/Chatblanccc/desktop-translate/actions/runs/29684078146)
  的 Phase 1–5 自动门禁全部通过，Phase 5 含 15 分钟 short soak；该 unsigned PR lane 不降低正式发布风险。
...
- [`6dae872…` clean unsigned Dir](../../artifacts/phase5/6dae872/clean-verify-local-20260719-final1/package/release/evidence-manifest.json)
  与 [clean unsigned Installer](../../artifacts/phase5/6dae872/clean-installer-local-20260719-final1/release/evidence-manifest.json)
  均记录 `developmentDirty=false`，production build、startup、SBOM/provenance、exact hash 与体积门禁通过。
  Installer canonical name 为 `Desktop-Translate-0.5.0-phase5-x64-setup.exe`，大小 `92004551` bytes，SHA-256
  为 `7886c7926d640c8e03a60350361a3f74f6dde1cae73f92ccf0a4de6d25a8a550`；应用、Native Host、Installer
  均 `NotSigned`、`acceptanceEligible=false`。P5-R-005 在 clean unsigned package 范围受控；P5-R-016、
  signed RC、clean VM、attestation 和 clean-download 仍阻塞发布。
