...
当前代码基线 `6dae872fa7b7fbabe4e74b77b351c3390170d77a` 的 clean-HEAD `pnpm phase5:verify` 已完整退出 `0`，
...
311 项，310 passed、1 skipped，coverage 通过。当前 clean unsigned Installer 的 canonical name 为
`Desktop-Translate-0.5.0-phase5-x64-setup.exe`，大小 `92004551` bytes，SHA-256 为
`7886c7926d640c8e03a60350361a3f74f6dde1cae73f92ccf0a4de6d25a8a550`；应用、Native Host 与 Installer
均为 `NotSigned`、`acceptanceEligible=false`，release 仍为 `RELEASE BLOCKED`。`6dae872…` 的远程
[remote run 29684078146](https://github.com/Chatblanccc/desktop-translate/actions/runs/29684078146) 的 Phase 1–5
自动门禁全部通过；Phase 5 的 15 分钟 deterministic short soak、mutation check 与 evidence upload 均成功。
...
| 对 Phase 5 的影响 | `BASELINE GAP, NOT CURRENT REGRESSION BLOCKER` | `6dae872…` clean Phase 5 严格超集已经通过；旧失败仍阻止 Phase 4→5 正式相对性能结论，但不再阻止开发回归继续 |
...
| 全仓 typecheck | `CLEAN-HEAD DEVELOPMENT PASS` | `6dae872…` clean run 通过 |
| Lint | `CLEAN-HEAD DEVELOPMENT PASS` | `6dae872…` clean run 通过 |
...
| 完整 Phase 4 strict-superset gate | `CLEAN-HEAD DEVELOPMENT PASS` | `6dae872…` clean `phase5:verify` 内完整执行；Electron E2E 6/6、Phase 2 3/3、Native 2/2 |
| 完整 Phase 5 deterministic gate | `LOCAL + REMOTE PASS NOT ACCEPTANCE` | [`6dae872…` summary](../../artifacts/phase5/6dae872/clean-verify-local-20260719-final1/verify-summary.json) 为 `strictPhase4Superset=true`、`worktreeDirty=false`、`acceptance=false`；[remote run 29684078146](https://github.com/Chatblanccc/desktop-translate/actions/runs/29684078146) 的 15 分钟 deterministic short soak、mutation check 与 evidence upload 均成功 |
...
|---|---|---|---|
| [`6dae872…` clean deterministic verify](../../artifacts/phase5/6dae872/clean-verify-local-20260719-final1/verify-summary.json) | `DETERMINISTIC_GATE_PASS_NOT_ACCEPTANCE` | clean HEAD、Phase 4 strict superset、unsigned package、audit、residual 与 privacy gate 通过 | Phase 5 acceptance、signed RC、formal PERF/RES、Lane 或兼容矩阵；`acceptance=false` |
| [`6dae872…` clean unsigned Dir package](../../artifacts/phase5/6dae872/clean-verify-local-20260719-final1/package/release/evidence-manifest.json) | `DEVELOPMENT PACKAGE PASS / RELEASE BLOCKED` | `developmentDirty=false`，package/startup/supply-chain PASS；installed 322.15 MiB | signed RC 或 clean VM；应用/Host `NotSigned`、`acceptanceEligible=false` |
...
| [历史 dirty deterministic verify](../../artifacts/phase5/local/acceptance-verify-rerun2-20260718-2300/verify-summary.json) | `DEVELOPMENT_GATE_PASS_NOT_ACCEPTANCE` | 未跳过 Phase 4 或 packaging；strict superset、unsigned package、audit、process/privacy、residual 与证据隐私链路退出 `0` | 当前结论或 Phase 5 acceptance；summary 明确记录 `worktreeDirty=true`、`acceptance=false` |
...
| [历史 dirty no-SkipBuild Dir 开发包](../../artifacts/phase5/local/acceptance-dir-rerun-20260718-2240/release/evidence-manifest.json) | `HISTORICAL DEVELOPMENT PACKAGE PASS` | production build、startup/D8、SBOM/provenance、exact hash 与体积门禁通过 | 当前候选或 release；manifest 为 dirty `HEAD+WORKTREE` |
| [`6dae872…` clean unsigned Installer](../../artifacts/phase5/6dae872/clean-installer-local-20260719-final1/release/evidence-manifest.json) | `DEVELOPMENT PACKAGE PASS / RELEASE BLOCKED` | canonical Installer `Desktop-Translate-0.5.0-phase5-x64-setup.exe`，`92004551` bytes，SHA-256 `7886c7926d640c8e03a60350361a3f74f6dde1cae73f92ccf0a4de6d25a8a550`；应用 hash `5536f2a26226ac038ac7bcc64814aa306e66e483e90b15986a76c0afd8aaf3b7`，Native hash `8a8d8591524693933486f9497e4b2240b51eea0d813d1030c7fffd722ff94077` | signed RC、attestation/clean-download 或 clean VM；三者均 `NotSigned`、`acceptanceEligible=false` |
...
| 正式 gate | 状态 | 缺失证据 |
|---|---|---|
| Phase 4 strict-superset rerun | `LOCAL + REMOTE DEVELOPMENT PASS` | `6dae872…` clean 本地完整退出 `0`；[remote run 29684078146](https://github.com/Chatblanccc/desktop-translate/actions/runs/29684078146) 的 Phase 1–5 自动门禁通过。独立 instrumentation-only Phase 4 baseline 仍是另一项 gap |
...
| no-SkipBuild package | `CLEAN UNSIGNED DIR/INSTALLER PASS / RELEASE BLOCKED` | `6dae872…` clean Dir 与 unsigned Installer 开发门禁通过；三项发布二进制均 `NotSigned`、`acceptanceEligible=false`。签名、attestation、clean-download 和 clean VM 未完成 |
...
`6dae872…` clean-HEAD 的 lint、全仓 typecheck、workspace coverage、workspace 400 discovered / 399 passed /
1 skipped / 0 failed、Desktop 34 files / 311 discovered / 310 passed / 1 skipped、Native 2/2、
...
开发期证据，但都不是 acceptance。`6dae872…` 的 [remote run 29684078146](https://github.com/Chatblanccc/desktop-translate/actions/runs/29684078146)
已通过全部自动门禁；固定机 PERF-01–09、
